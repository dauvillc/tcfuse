...
def to_compact_time(
    snapshot_time_utc: str | pd.Timestamp | float | int,
    *,
    unit: str | None = None,
) -> str:
    ...
    Args:
        snapshot_time_utc: ISO 8601 timestamp, pandas Timestamp, or numeric epoch
    ...
    """
    if unit is not None:
        ts = pd.Timestamp(snapshot_time_utc, unit=unit)
    else:
        ts = pd.Timestamp(snapshot_time_utc)
    if not isinstance(ts, pd.Timestamp):
        raise ValueError(f"Invalid timestamp: {snapshot_time_utc!r}")
    return ts.strftime(_COMPACT_TIME_FMT)


def lead_hours_rounded(init_time_utc: str, snapshot_time_utc: str) -> int:
    ...
    Args:
        ...
        snapshot_time_utc: Snapshot valid time as ISO 8601.

    Returns:
        Rounded lead time in hours.
    """
    init_ts = pd.Timestamp(init_time_utc)
    snap_ts = pd.Timestamp(snapshot_time_utc)
