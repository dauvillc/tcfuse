"""Decoded data containers for MoTiF: per-source value tensors after un-embedding.

Coord-free by construction: the MoTiF decoders reconstruct values only, from the
``MotifEmbeddedSource.values`` token tensor (the coordinate tokens are ignored).
Duplicated from :mod:`tcfuse.models.decoders.decoded` so the MoTiF un-embedding
stack stays fully self-contained, mirroring how ``encoders_motif`` duplicates the
embedded containers.
"""
...
@dataclass
class MotifDecodedSource:
    """A single source after un-embedding MoTiF value tokens back into raw values.

    Produced by a :class:`~tcfuse.models.decoders_motif.base.MotifSourceDecoder`
    from a :class:`~tcfuse.models.encoders_motif.embedded.MotifEmbeddedSource`
    (using only its ``values``). Mirrors the value layout of
    :class:`~tcfuse.data.sources.torch_source.TorchSource` (not the embedded token
    layout): it carries no coords / mask / time, since the decoder reconstructs
    values only.
    ...
    def to(self, device: torch.device | str) -> MotifDecodedSource:
        """Move ``values`` to ``device``, returning a new MotifDecodedSource."""
...
@dataclass
class MotifDecodedBatch:
    """Batched collection of decoded sources, the un-embedding counterpart of MotifEmbeddedBatch.

    Args:
        sources: Dict from ``(source_name, source_index)`` to a MotifDecodedSource.
        ...
        is_target: Dict from ``(source_name, source_index)`` to a ``(B,)`` bool
            ...
            through unchanged from the originating MotifEmbeddedBatch.
    """

    sources: dict[tuple[str, int], MotifDecodedSource]
