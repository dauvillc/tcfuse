"""Standalone spatio-temporal Fourier coordinate embedding for MoTiF.
...
the concatenated features to a dedicated ``coord_dim`` with a learnable linear
layer. Unlike the additive positional encoding in
:mod:`tcfuse.models.encoders.positional`, the output here is a **standalone**
coordinate-embedding tensor, kept separate from the value embedding so the
...
class MotifCoordEncodingConfig:
    """Hyperparameters for :class:`CoordEmbedding`.

    Bundled into one object so backbones can thread a single config through to
    the embedding layer instead of many loose scalars. There is no ``enabled``
    switch: a separate coordinate embedding is the premise of the MoTiF
    architecture, so the coord tensor is always produced.
...
class CoordEmbedding(nn.Module):
    """Standalone Fourier coordinate embedding over spatial + temporal coordinates.

    Builds, per coordinate axis, a fixed bank of sin/cos features from log-spaced
    frequencies, concatenates them across axes, and projects to ``coord_dim`` with
    a single learnable linear layer. The output is a standalone coordinate tensor
    (never added to the value embedding), consumed by the MoTiF backbone as
    positional conditioning at every layer.
    ...
    def __init__(
        ...
        *,
        ...
        config: MotifCoordEncodingConfig,
    ) -> None:
        super().__init__()
        ...
        # Project the concatenated Fourier features to the coordinate-embedding dim.
        self.proj = nn.Linear(num_features, coord_dim)

    def forward(self, coords: Tensor, time: Tensor) -> Tensor:
        """Encode per-token coordinates and per-sample time into ``(..., Dc)``.
        ...
        Returns:
            Coordinate embedding of shape ``(B, *spatial, Dc)``, kept separate
            from the value embedding.
        """
        ...
        # Zero NaN-fill at missing/masked slots so the embedding stays finite; those
        ...
        # Project to the coordinate-embedding dimension.
